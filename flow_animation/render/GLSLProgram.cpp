...
GLSLProgram::GLSLProgram() : handle(0), vaoHandle(0), linked(false) {}
...
void GLSLProgram::createContex(HWND hWnd) {
	...
	if (PixelFormat == 0) throw GLSLProgramException("Can't choose pixel format");
	if (!SetPixelFormat(hdc, PixelFormat, &pfd)) throw std::runtime_error("Can't set pixel format");
...
void GLSLProgram::draw(GLenum mode, GLint first, GLsizei count) {
...
void GLSLProgram::setAtribute(GLuint location, const std::vector<float> &data, GLint size) {
	GLuint buffer;
...
void GLSLProgram::PrintGPUVersion(bool display_extensions) {
	const GLubyte *renderer = glGetString(GL_RENDERER);
	const GLubyte *vendor = glGetString(GL_VENDOR);
	const GLubyte *version = glGetString(GL_VERSION);
	const GLubyte *glsh_version = glGetString(GL_SHADING_LANGUAGE_VERSION);
	...
	std::printf("GL Version (string)    : %s\n", version);
	std::printf("GL Version (integer)   : %d.%d\n", major, minor);
