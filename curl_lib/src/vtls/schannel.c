...
int Curl_schannel_shutdown(struct connectdata *conn, int sockindex)
{
  ...
  if(connssl->cred && connssl->ctxt) {
    ...
    CURLcode result;
    TCHAR *host_name;
    unsigned long dwshut = SCHANNEL_SHUTDOWN;
...
int Curl_schannel_random(unsigned char *entropy, size_t length)
{
  ...
  if(!CryptGenRandom(hCryptProv, (unsigned long)length, entropy)) {
...
static CURLcode verify_certificate(struct connectdata *conn, int sockindex)
{
  ...
  if(result == CURLE_OK) {
    ...
    if(result == CURLE_OK) {
      ...
      unsigned long dwTrustErrorMask = ~(unsigned long)(CERT_TRUST_IS_NOT_TIME_NESTED);
